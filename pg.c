	...
	*/
	...
	for(int cnt = 0; cnt < rows_per_proc; cnt++) {
		...
		for(int i = prev_curr + 1; i < curr; i++) {
			...
			printf("@%d, i = %d, curr = %d, prev_curr = %d, Array received from %d: ", id, i, curr, prev_curr, prev_proc);
			...
			*/
			if(curr - i < num_proc - 1) {
				...
				printf("@%d, i = %d, curr = %d, prev_curr = %d, Array sent to %d: ", id, i, curr, prev_curr, next_proc);
				print_arr(recvd_row, num_eq + 2, fptr);
			}
